...
namespace simulator = qssc::targets::simulator;
...
auto simulator::SimulatorQubitLocalizationPass::lookupQubitId(const Value &val) -> int {
...
/// Creates a broadcast op on Controller and recvOp on all other simulators
/// also checks if a value *should* be broadcast
void simulator::SimulatorQubitLocalizationPass::broadcastAndReceiveValue(
  ...
  if (alreadyBroadcastValues.count(val) == 0) {
    ...
    if (parentOp) {
      if (dyn_cast<mlir::arith::ConstantOp>(parentOp) ||
        ...
        // Just clone this op to the simulators
        for (uint id : toNodeIds)
          (*simulatorBuilders)[id]->clone(*parentOp, simulatorMapping[id]);
      } else {
        controllerBuilder->create<BroadcastOp>(
            loc, controllerMapping.lookupOrNull(val));
        for (uint id : toNodeIds) {
          auto recvOp = (*simulatorBuilders)[id]->create<RecvOp>(
              loc, TypeRange(val.getType()),
              controllerBuilder->getIndexArrayAttr(config->controllerNode()));
          simulatorMapping[id].map(val, recvOp.vals().front());
        }
      }
...
void simulator::SimulatorQubitLocalizationPass::cloneRegionWithoutOps(
...
auto simulator::SimulatorQubitLocalizationPass::addMainFunction(
...
void simulator::SimulatorQubitLocalizationPass::processOp(DeclareQubitOp &qubitOp) {
  Operation *op = qubitOp.getOperation();
  llvm::outs() << "Localizing a " << op->getName() << "\n";

  // declare every qubit on each simulator for multi-qubit gates purposes
  for (auto nodeId : seenNodeIds) {
    auto *clonedOp = (*simulatorBuilders)[nodeId]->clone(*op);
    simulatorMapping[nodeId].map(qubitOp.res(),
                            dyn_cast<DeclareQubitOp>(clonedOp).res());
  }
} // processOp DeclareQubitOp

void simulator::SimulatorQubitLocalizationPass::processOp(ResetQubitOp &resetOp) {
  ...
  (*simulatorBuilders)[config->driveNode(qubitId)]->clone(
      *op, simulatorMapping[config->driveNode(qubitId)]);
  (*simulatorBuilders)[config->acquireNode(qubitId)]->clone(
      *op, simulatorMapping[config->acquireNode(qubitId)]);
} // processOp ResetQubitOp

void simulator::SimulatorQubitLocalizationPass::processOp(FuncOp &funcOp) {
...
void simulator::SimulatorQubitLocalizationPass::processOp(Builtin_UOp &uOp) {
  Operation *op = uOp.getOperation();
  llvm::outs() << "Localizing a " << op->getName() << "\n";
  int qubitId = lookupQubitId(uOp.target());

  // broadcast all classical values from Controller to all Simulatorss
  // recv all classical values on all Simulatorss
  ...
  // clone the gate call to the drive simulator
  (*simulatorBuilders)[config->driveNode(qubitId)]->clone(
      *op, simulatorMapping[config->driveNode(qubitId)]);
} // processOp Builtin_UOp

void simulator::SimulatorQubitLocalizationPass::processOp(BuiltinCXOp &cxOp) {
  ...
  // clone the gate call to the drive simulators
  (*simulatorBuilders)[config->driveNode(qubitId1)]->clone(
      *op, simulatorMapping[config->driveNode(qubitId1)]);
  (*simulatorBuilders)[config->driveNode(qubitId2)]->clone(
      *op, simulatorMapping[config->driveNode(qubitId2)]);
} // processOp BuiltinCXOp

void simulator::SimulatorQubitLocalizationPass::processOp(MeasureOp &measureOp) {
  ...
  // clone the measure call to the drive and acquire simulators
  (*simulatorBuilders)[config->driveNode(qubitId)]->clone(
      *op, simulatorMapping[config->driveNode(qubitId)]);
  Operation *clonedOp = (*simulatorBuilders)[config->acquireNode(qubitId)]->clone(
      *op, simulatorMapping[config->acquireNode(qubitId)]);
  auto clonedMeasureOp = dyn_cast<MeasureOp>(clonedOp);

  // send the results from the acquire simulator and recv on Controller
  (*simulatorBuilders)[config->acquireNode(qubitId)]->create<SendOp>(
...
void simulator::SimulatorQubitLocalizationPass::processOp(
  ...
  if (!onlyToController) {
    ...
    // Clone the subroutine call to all drive and acquire simulators
    for (uint nodeId : seenNodeIds) {
      (*simulatorBuilders)[nodeId]->clone(*op, simulatorMapping[nodeId]);
  ...
  if (!onlyToController) {
    for (uint nodeId : seenNodeIds) {
      savedPoint = (*simulatorBuilders)[nodeId]->saveInsertionPoint();
      (*simulatorBuilders)[nodeId]->setInsertionPointToStart(
          dyn_cast<ModuleOp>(simulatorModules[nodeId]).getBody());
      Operation *clonedFuncOperation =
          (*simulatorBuilders)[nodeId]->cloneWithoutRegions(*funcOperation,
                                                       simulatorMapping[nodeId]);
      auto clonedFuncOp = dyn_cast<FuncOp>(clonedFuncOperation);
      if (funcOp.getCallableRegion()) {
        cloneRegionWithoutOps(&funcOp.getBody(), &clonedFuncOp.getBody(),
                              simulatorMapping[nodeId]);
      ...
      (*simulatorBuilders)[nodeId]->restoreInsertionPoint(savedPoint);
    } // for nodeId in seenNodeIds
  }   // if !onlyToController
...
void simulator::SimulatorQubitLocalizationPass::processOp(CallGateOp &callOp) {
  Operation *op = callOp.getOperation();
  llvm::outs() << "Localizing a " << op->getName() << "\n";
  ...
  // Clone the gate call to all relevant drive simulators
...
void simulator::SimulatorQubitLocalizationPass::processOp(BarrierOp &barrierOp) {
...
void simulator::SimulatorQubitLocalizationPass::processOp(CallDefCalGateOp &callOp) {
  Operation *op = callOp.getOperation();
  llvm::outs() << "Localizing a " << op->getName() << "\n";
  // first find out which qubits this call operates on
  // and what non-qubit values it uses
  ...
  // send all classical values from Controller to all Simulatorss
  // recv all classical values on all drive Simulatorss
  for (Value val : classicalVals)
    broadcastAndReceiveValue(val, op->getLoc(), seenNodeIds);
...
void simulator::SimulatorQubitLocalizationPass::processOp(CallDefcalMeasureOp &callOp) {
  Operation *op = callOp.getOperation();
  llvm::outs() << "Localizing a " << op->getName() << "\n";
  // first find out which qubits this call operates on
  // and what non-qubit values it uses
  ...
  for (uint qubitId : qInd) {
    // Clone the measure call to all drive and acquire simulators
    (*simulatorBuilders)[config->driveNode(qubitId)]->clone(
        *op, simulatorMapping[config->driveNode(qubitId)]);
...
void simulator::SimulatorQubitLocalizationPass::processOp(DelayOpType &delayOp) {
  ...
  if (auto dOp = dyn_cast<DelayOp>(op)) {
    ...
    for (uint id : involvedNodes)
      (*simulatorBuilders)[id]->clone(*durationDeclare, simulatorMapping[id]);
  }
...
void simulator::SimulatorQubitLocalizationPass::processOp(mlir::ReturnOp &returnOp) {
...
void simulator::SimulatorQubitLocalizationPass::processOp(scf::YieldOp &yieldOp) {
...
void simulator::SimulatorQubitLocalizationPass::processOp(
  ...
  // first broadcast the condition value from Controller to Simulatorss
  ...
  if (measureOp) { // remove the drive node from seenNodeIds temporarily
    ...
    if (savedQubitId >= 0) {
      ...
      auto recvOp =
          (*simulatorBuilders)[config->driveNode(savedQubitId)]->create<RecvOp>(
      ...
      simulatorMapping[config->driveNode(savedQubitId)].map(ifOp.getCondition(),
                                                       recvOp.vals().front());
    }
  }
  ...
  for (uint nodeId : seenNodeIds) {
    Operation *clonedOp =
        (*simulatorBuilders)[nodeId]->cloneWithoutRegions(*op, simulatorMapping[nodeId]);
    auto clonedIfOp = dyn_cast<scf::IfOp>(clonedOp);
    if (!ifOp.getThenRegion().empty()) {
      cloneRegionWithoutOps(&ifOp.getThenRegion(), &clonedIfOp.getThenRegion(),
                            simulatorMapping[nodeId]);
...
void simulator::SimulatorQubitLocalizationPass::processOp(
  ...
  if (classicalOnlyCheck(op)) {
    ...
    // from Controller to Simulatorss then clone the for op everywhere
    ...
    cloneRegionWithoutOps(&forOp.getLoopBody(), &clonedForOp.getLoopBody(),
                          controllerMapping);
    for (uint nodeId : seenNodeIds) {
      Operation *clonedOp = (*simulatorBuilders)[nodeId]->cloneWithoutRegions(
          *op, simulatorMapping[nodeId]);
...
void simulator::SimulatorQubitLocalizationPass::runOnOperation(SimulatorSystem &target) {
  ...
  mainFunc->walk([&](DeclareQubitOp qubitOp) {
    ...
    if (!simulatorModules.count(config->driveNode(qubitId))) {
      llvm::outs() << "Creating module for drive Simulators " << qubitId << "\n";
      auto driveMod = b.create<ModuleOp>(
          qubitOp->getLoc(),
          llvm::StringRef("simulator_drive_" + std::to_string(qubitId)));
      ...
      simulatorModules[config->driveNode(qubitId)] = driveMod.getOperation();
      FuncOp simulatorMainOp =
          addMainFunction(driveMod.getOperation(), mainFunc->getLoc());
      newBuilders->emplace(config->driveNode(qubitId),
                           new OpBuilder(simulatorMainOp.getBody()));
    }

    if (!simulatorModules.count(config->acquireNode(qubitId))) {
      llvm::outs() << "Creating module for acquire Simulators " << qubitId << "\n";
      auto acquireMod = b.create<ModuleOp>(
          qubitOp->getLoc(),
          llvm::StringRef("simulator_acquire_" + std::to_string(qubitId)));
      ...
      simulatorModules[config->acquireNode(qubitId)] = acquireMod.getOperation();
      FuncOp simulatorMainOp =
  ...
  while (!blockAndBuilderWorkList.empty()) {
    ...
    simulatorBuilders = std::get<2>(blockAndBuilderWorkList.front()).release();
    ...
    for (uint nodeId : seenNodeIds)
      delete (*simulatorBuilders)[nodeId];
    delete simulatorBuilders;
  } // while !blockAndBuilderWorklist.empty()

  cloneVariableDeclarations(topModuleOp);
} // runOnOperation()

void simulator::SimulatorQubitLocalizationPass::cloneVariableDeclarations(
...
llvm::StringRef SimulatorQubitLocalizationPass::getArgument() const {
  return "simulator-qubit-localization";
}

llvm::StringRef SimulatorQubitLocalizationPass::getDescription() const {
  return "Create modules for Simulator code blocks.";
}
