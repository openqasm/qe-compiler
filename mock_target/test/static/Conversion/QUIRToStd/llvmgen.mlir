...
module @controller attributes {quir.nodeId = 1000 : i32, quir.nodeType = "controller"}  {
  func @main() -> i32 attributes {quir.classicalOnly = false} {
    ...
    %1 = qusys.recv {fromId = 0 : index} : i1
    qusys.broadcast %1 : i1
    scf.if %1 {
    } {quir.classicalOnly = false}
    %2 = qusys.recv {fromId = 0 : index} : i1
    qusys.broadcast %2 : i1
    scf.if %2 {
    } {quir.classicalOnly = false}
    %3 = qusys.recv {fromId = 0 : index} : i1
    qusys.broadcast %3 : i1
    scf.if %3 {
    } {quir.classicalOnly = false}
