...
module @acquire_0 attributes {quir.nodeId = 7 : i32, quir.nodeType = "acquire", quir.physicalIds = [0 : i32, 1 : i32, 2 : i32, 3 : i32]} {
  pulse.sequence @seq_0(%arg0: !pulse.mixed_frame, %arg1: !pulse.mixed_frame, %arg2: !pulse.mixed_frame, %arg3: !pulse.mixed_frame, %arg4: !pulse.mixed_frame) -> i1 {
    ...
    // CHECK-NOT: pulse.delay {timepoint = 0 : i64}(%[[ARG1]], %c4336_i32) : (!pulse.mixed_frame, i32)
    // CHECK-NOT: pulse.delay {timepoint = 4336 : i64}(%[[ARG0]], %c160_i32) : (!pulse.mixed_frame, i32)
    // CHECK: %0 = pulse.capture {timepoint = 4496 : i64}(%[[ARG0]]) : (!pulse.mixed_frame) -> i1
    // CHECK-NOT: pulse.delay {timepoint = 4496 : i64}(%{{.*}}, %c13600_i32) : (!pulse.mixed_frame, i32)
    ...
    // CHECK-NOT: pulse.delay {timepoint = 4496 : i64}(%{{.*}}, %c18096_i32) : (!pulse.mixed_frame, i32)
    // CHECK-NOT: pulse.delay {timepoint = 4496 : i64}(%{{.*}}, %c18096_i32) : (!pulse.mixed_frame, i32)
    // CHECK-NOT: pulse.delay {timepoint = 4496 : i64}(%{{.*}}, %c18096_i32) : (!pulse.mixed_frame, i32)
    // CHECK: pulse.return {timepoint = 18096 : i64} %{{.*}} : i1
    pulse.return %0 : i1
  }
  func @main() -> i32 attributes {quir.classicalOnly = false} {
    ...
    %9 = "pulse.mix_frame"(%7, %8) {signal_type = "drive"} : (!pulse.port, !pulse.frame) -> !pulse.mixed_frame
    %10 = "pulse.create_port"() {uid = "Q2"} : () -> !pulse.port
    %11 = pulse.create_frame(%0, %cst, %1) : (complex<f64>, f64, !quir.angle<20>) -> !pulse.frame
    %12 = "pulse.mix_frame"(%10, %11) {signal_type = "drive"} : (!pulse.port, !pulse.frame) -> !pulse.mixed_frame
    %13 = "pulse.create_port"() {uid = "Q3"} : () -> !pulse.port
    %14 = pulse.create_frame(%0, %cst, %1) : (complex<f64>, f64, !quir.angle<20>) -> !pulse.frame
    %15 = "pulse.mix_frame"(%13, %14) {signal_type = "drive"} : (!pulse.port, !pulse.frame) -> !pulse.mixed_frame
