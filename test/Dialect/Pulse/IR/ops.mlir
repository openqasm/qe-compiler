...
func @main () {
    ...
    pulse.mix_frame(%d0, %f0) : (!pulse.port, !pulse.frame)
    // CHECK: pulse.mix_frame(%[[D0]], %[[F0]]) : (!pulse.port, !pulse.frame)
    ...
    %res0, %res1 = pulse.call_sequence @test_pulse_ops (%d0, %m0, %f0, %param_amp) : (!pulse.port, !pulse.port, !pulse.frame, complex<f64>) -> (i1, i1)
    // CHECK: %{{.*}}:2 = pulse.call_sequence @test_pulse_ops(%[[D0]], %[[M0]], %[[F0]], %[[AMP]]) : (!pulse.port, !pulse.port, !pulse.frame, complex<f64>) -> (i1, i1)

    return
}

pulse.sequence @test_pulse_ops (%d0: !pulse.port, %m0: !pulse.port, %f0: !pulse.frame, %amp: complex<f64>) -> (i1, i1) {
// CHECK: pulse.sequence @test_pulse_ops(
    ...
    // CHECK-SAME: %[[ARG3:[A-Za-z0-9]+]]: complex<f64>) -> (i1, i1) {
    ...
    %summed = pulse.sum(%drag_waveform, %drag_waveform) : (!pulse.waveform, !pulse.waveform) -> !pulse.waveform
    // CHECK %{{.*}} = pulse.sum(%[[DRAG]], %[[DRAG]]) : (!pulse.waveform, !pulse.waveform) -> !pulse.waveform

    %fc = arith.constant 200.e04 : f64
    pulse.set_frequency(%d0, %fc) : (!pulse.port, f64)
    // CHECK pulse.set_frequency(%[[ARG0]], %cst_0) : (!pulse.port, f64)
    pulse.set_frequency(%f0, %fc) : (!pulse.frame, f64)
    // CHECK pulse.set_frequency(%[[ARG2]], %cst_0) : (!pulse.frame, f64)

    pulse.shift_frequency(%d0, %fc) : (!pulse.port, f64)
    // CHECK pulse.shift_frequency(%[[ARG0]], %cst_0) : (!pulse.port, f64)
    ...
    pulse.set_phase(%d0, %angle) : (!pulse.port, f64)
    // CHECK pulse.set_phase(%[[ARG0]], %cst_1) : (!pulse.port, f64)
    pulse.set_phase(%f0, %angle) : (!pulse.frame, f64)
    // CHECK pulse.set_phase(%[[ARG2]], %cst_1) : (!pulse.frame, f64)

    pulse.shift_phase(%d0, %angle) : (!pulse.port, f64)
    // CHECK pulse.shift_phase(%[[ARG0]], %cst_1) : (!pulse.port, f64)
    ...
    pulse.delay(%delay_duration, %d0) : (i32, !pulse.port)
    // CHECK pulse.delay(%c100_i32, %arg0) : (i32, !pulse.port)

    pulse.play(%d0, %drag_waveform) : (!pulse.port, !pulse.waveform)
    // CHECK pulse.play(%[[ARG0]], %2) : (!pulse.port, !pulse.waveform)
    pulse.play(%f0, %drag_waveform) : (!pulse.frame, !pulse.waveform)
    // CHECK pulse.play(%[[ARG2]], %2) : (!pulse.frame, !pulse.waveform)
    ...
    %res0 = pulse.capture(%m0, %kernel) : (!pulse.port, !pulse.kernel) -> i1
    // CHECK %[[RESULT:.*]] = pulse.capture(%[[ARG1]], %[[KERNEL]]) : (!pulse.port, !pulse.kernel) -> i1
