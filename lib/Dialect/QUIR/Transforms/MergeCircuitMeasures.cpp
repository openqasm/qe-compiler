...
static std::string duplicateCircuit(PatternRewriter &rewriter,
  ...
  symbolMap[circuitOp.sym_name()] = oldCircuitOp;
  ...
  int cnt = 0;
  while (symbolMap.find(testName) != symbolMap.end())
...
static void remapArguments(PatternRewriter &rewriter,
  ...
  // remap arguments
  for (auto argument : nextMeasureOp.qubits()) {
    auto blockArgument = argument.dyn_cast<BlockArgument>();
...
static void buildOutputs(llvm::SmallVector<Type> &outputTypes,
  ...
  auto result = mergedOp.outs().begin() + resultOffset;
...
static void dropNextMeasure(PatternRewriter &rewriter,
  ...
  auto opType = nextCircuitOp.getType();
...
struct CallCircuitAndCallCircuitTopologicalPattern
  ...
  LogicalResult matchAndRewrite(CallCircuitOp callCircuitOp,
    ...
    auto funcOp = dyn_cast<FuncOp>(callCircuitOp->getParentOp());
    ...
    if (isa<MeasureOp>(firstOp)) {
      ...
      if (!firstMeasureOpt.hasValue())
        return failure();
      firstMeasureOp = firstMeasureOpt.getValue();
    ...
    if (isa<MeasureOp>(firstOp)) {
      ...
      if (!nextMeasureOpt.hasValue())
        return failure();
      nextMeasureOp = nextMeasureOpt.getValue();
    }
