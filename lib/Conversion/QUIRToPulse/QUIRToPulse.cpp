...
void QUIRToPulsePass::convertCircuitToSequence(CallCircuitOp callCircuitOp,
  ...
  SequenceOp convertedPulseSequenceOp = builder.create<mlir::pulse::SequenceOp>(
  ...
  circuitOp->walk([&](Operation *quirOp) {
    ...
    } else if (!(isa<quir::ConstantOp>(quirOp) or isa<quir::ReturnOp>(quirOp) or
...
void QUIRToPulsePass::processCircuitArgs(
  ...
  for (uint cnt = 0; cnt < circuitOp.getNumArguments(); cnt++) {
    ...
    if (argumentType.isa<mlir::quir::AngleType>()) {
      ...
      assert(false && "unkown circuit argument.");
  }
}
...
void QUIRToPulsePass::processPulseCalArgs(
  ...
  for (auto const &argumentResult :
    ...
    if (argumentType.isa<WaveformType>()) {
      ...
      if (argAttr[index].dyn_cast<StringAttr>().getValue().str() != "angle")
      ...
      if (argAttr[index].dyn_cast<StringAttr>().getValue().str() != "duration")
        assert(false && "unkown argument.");
      ...
      assert(false && "unkown argument type.");
  }
}
...
void QUIRToPulsePass::processMixFrameOpArg(
    std::string mixFrameName, std::string portName,
...
void QUIRToPulsePass::processPortOpArg(std::string portName,
                                       SequenceOp convertedPulseSequenceOp,
                                       SmallVector<Value> &pulseCalSequenceArgs,
...
void QUIRToPulsePass::processWfrOpArg(std::string wfrName,
                                      SequenceOp convertedPulseSequenceOp,
                                      SmallVector<Value> &pulseCalSequenceArgs,
                                      Value argumentValue, FuncOp &mainFunc,
                                      mlir::OpBuilder &builder) {
  ...
  if (classicalQUIROpLocToConvertedPulseOpMap.find(angleLocHash) ==
    ...
    if (auto castOp = dyn_cast<quir::ConstantOp>(angleOp)) {
      ...
      if (auto paramCastOp =
        ...
        assert(false && "castOp arg unknown");
    } else
      assert(false && "angleOp unknown");
  }
...
mlir::Value QUIRToPulsePass::convertDurationToI64(
  ...
  if (classicalQUIROpLocToConvertedPulseOpMap.find(durLocHash) ==
    ...
    if (auto castOp = dyn_cast<quir::ConstantOp>(durationOp)) {
      ...
      assert(false && "unkown duration op");
  }
  return classicalQUIROpLocToConvertedPulseOpMap[durLocHash];
}

mlir::pulse::Port_CreateOp
QUIRToPulsePass::addPortOpToIR(std::string portName, FuncOp &mainFunc,
                               mlir::OpBuilder &builder) {
...
QUIRToPulsePass::addMixFrameOpToIR(std::string mixFrameName,
                                   std::string portName, FuncOp &mainFunc,
                                   mlir::OpBuilder &builder) {
...
QUIRToPulsePass::addWfrOpToIR(std::string wfrName, FuncOp &mainFunc,
                              mlir::OpBuilder &builder) {
...
void QUIRToPulsePass::parsePulseWaveformContainerOps(
  ...
  if (!waveformContainerFileModule)
    assert(false and "problem parsing waveform container file");
