...
void qssc::config::QSSConfig::emit(llvm::raw_ostream &os) const {
  ...
  os << "targetConfigPath: " << (targetConfigPath.has_value() ? targetConfigPath.value() : "None") << "\n";
  os << "allowUnregisteredDialects: " << allowUnregisteredDialects << "\n";
  os << "addTargetPasses: " << addTargetPasses << "\n";
}

llvm::raw_ostream &operator<<(llvm::raw_ostream &os, const qssc::config::QSSConfig &config) {
  config.emit(os);
  return os;
}

std::ostream &operator<<(std::ostream& os, const qssc::config::QSSConfig &config) {
