// RUN: touch test.cfg
// RUN: echo "{}" >> test.cfg
// RUN: qss-compiler -X=qasm --emit=mlir --target aer-simulator --config test.cfg %s --num-shots=1 --break-reset --simulator-output-cregs --quir-eliminate-variables --simulator-quir-to-aer | FileCheck %s
