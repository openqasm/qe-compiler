// RUN: touch test.cfg
// RUN: echo "{}" >> text.cfg
// RUN: qss-compiler -X=mlir --emit=mlir --target aer-simulator --config test.cfg %s --simulator-quir-to-aer | FileCheck %s
...
module {
  ...
  func @main() -> i32 {
    ...
    // CHECK: llvm.call @aer_apply_u3({{.*}}, %[[q0]], {{.*}}, {{.*}}, {{.*}}) : (!llvm.ptr<i8>, i64, f64, f64, f64) -> ()
